...
\articletype{REVISED MANUSCRIPT (MARKED-UP VERSION -- REVISIONS HIGHLIGHTED IN BLUE)}
...
\affil{Manuscript (Ref.: Ms. No. 266350252) submitted to Journal of Information and Telecommunication (JIT)}
}
...
Deep convolutional networks have long dominated automated plant disease recognition \citep{mohanty2016using}. Widely adopted architectures including VGG \citep{ferentinos2018deep}, ResNet \citep{he2016deep, sethy2020deep, singh2024comprehensive}, EfficientNet \citep{tan2019efficientnet}, and MobileNet \citep{chen2021identification, too2019comparative} have achieved classification accuracies surpassing 97\% on laboratory benchmarks, even under resource-constrained conditions \citep{batool2025compact}. \textcolor{blue}{Furthermore, robust deep neural networks have been tailored for complex paddy field scenarios \citep{mookkandi2025robustdnn}, and local descriptor-based approaches have shown efficacy \citep{mookkandi2026robust}. Broadly, recent reviews emphasize the critical need to resolve structural bottlenecks in AI-driven agricultural disease identification \citep{nath2026bottleneck}.} Nevertheless, standard CNNs remain fundamentally constrained by the inductive bias of localized grid convolutions \citep{lecun1998gradient}, creating difficulties when aggregating spatially disconnected, fine-grained lesion symptoms under natural field conditions. Furthermore, most CNN approaches depend heavily on ImageNet pre-trained representations.

\subsection{Vision Transformers}
Vision Transformers (ViTs) \citep{dosovitskiy2020image} overcome local spatial limitations by employing global self-attention across image patch tokens. Recent variants such as DeiT \citep{touvron2021training}, Swin Transformer \citep{liu2021swin}, and hybrid CNN-ViT architectures \citep{yu2025stcfi} have improved efficiency and multi-scale feature extraction. However, ViTs require massive training corpora and lack spatial inductive biases, making them prone to severe optimization instability and over-fitting when trained from scratch on agricultural datasets at low resolutions \citep{borhani2022deep}. \textcolor{blue}{To mitigate these issues, lightweight and robust Vision Transformer variants have recently been introduced specifically for crop disease classification \citep{mookkandi2025robustvit}.}
...
\textcolor{blue}{Operating at $64\times64$ resolution enables real-time edge processing on modest hardware platforms. Although standard residual CNNs (e.g., ResNet-18) achieve higher raw inference throughput (439.9 FPS) due to static, localized grid convolutions, their rigid receptive fields suffer severe accuracy degradation under complex natural field noise and scattered lesions (dropping to 79.64\% on PlantRaw). In contrast, ViGATs maintains a real-time throughput of 74.0 FPS (well exceeding the standard 30 FPS threshold required for real-time robotic and UAV video feeds) while delivering a significant +6.48\% accuracy improvement on unconstrained field benchmarks (86.12\%) with only 129.8 MB peak VRAM. This demonstrates an optimal, deployment-ready trade-off between botanical diagnostic accuracy and edge computational throughput.}

\textcolor{blue}{While this study strictly adopts a $64\times64$ input resolution to faithfully emulate resource-constrained edge IoT sensor nodes and low-bandwidth UAV platforms under a train-from-scratch setting, the isotropic GATConv architecture is fundamentally resolution-agnostic. In cloud-based or high-performance computing (HPC) deployment scenarios, processing higher-resolution inputs ($224\times224$ or $384\times384$) can capture finer micro-pathological textures. To alleviate the computational overhead of dynamic graph construction at larger token counts ($N$), hierarchical patch merging, local windowed graph partitions, or approximate nearest neighbor (ANN) search algorithms can be naturally integrated as a promising direction for future research.}
...
\bibitem[Mookkandi \& Nath(2025)]{mookkandi2025robustdnn}
\textcolor{blue}{Mookkandi, K., \& Nath, M. K. (2025). Robust Deep Neural Network for Classification of Diseases from Paddy Fields. \emph{AgriEngineering}, \emph{7}(7), 0205. \url{https://doi.org/10.3390/agriengineering7070205}.}

\bibitem[Mookkandi et~al.(2025)]{mookkandi2025robustvit}
\textcolor{blue}{Mookkandi, K., Nath, M. K., Dash, S. S., Mishra, M., \& Blange, R. (2025). A Robust Lightweight Vision Transformer for Classification of Crop Diseases. \emph{AgriEngineering}, \emph{7}(8), 0268. \url{https://doi.org/10.3390/agriengineering7080268}.}

\bibitem[Mookkandi et~al.(2026)]{mookkandi2026robust}
\textcolor{blue}{Mookkandi, K., Nath, M. K., \& Balakrishnan, K. (2026). Robust method for classification of agricultural crops diseases using LGXP descriptor. \emph{Results in Engineering}. \url{https://doi.org/10.1016/j.rineng.2025.108826}.}

\bibitem[Nath et~al.(2026)]{nath2026bottleneck}
\textcolor{blue}{Nath, M. K., Mookkandi, K., Balakrishnan, K., Mishra, M., Kavitha, M., \& Mathumathi, M. (2026). Bottleneck of AI driven techniques for identifying indian agricultural diseases: A systematic review and future directives. \emph{Results in Engineering}. \url{https://doi.org/10.1016/j.rineng.2026.111212}.}
